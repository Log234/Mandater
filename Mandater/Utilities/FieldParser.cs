...
namespace Mandater.Utilities
{
    ...
    public class FieldParser
    {
        ...
        public string ParseString(string value, string field, int minLength, int maxLength)
        {
            ...
            {
                throw new CsvFileFormatException($"The {field} \"{value}\" is shorter than the minimum length. Expected a value >= {minLength}, got: {value.Length}", File, Line);
            }
            if (maxLength != -1 && value.Length > maxLength)
            {
                throw new CsvFileFormatException($"The field {field} \"{value}\" is longer than the maximum length. Expected a value < {maxLength}, got: {value.Length}", File, Line);
            }
            return value;
        }
    }
